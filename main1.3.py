...
class DocbildUI:
    def __init__(self, master):
        ...
        for i, (source_format, target_format) in enumerate(file_format_combinations):
            ...
            # Modify button text to display extension names in capital letters without the "."
            button_text = f"{source_format[1:].upper()} to {target_format[1:].upper()}"
            button = ttk.Button(self.master, text=button_text, command=lambda s=source_format, t=target_format: self.convert_file(s, t), width=15)
