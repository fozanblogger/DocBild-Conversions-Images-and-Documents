...
class DOCXtoDOTXConverterUI:
    def __init__(self, master):
        self.master = master
        self.master.title("DOCX to DOTX")  # Updated title
        ...
        # Create widgets
        self.heading_label = tk.Label(self.master, text="DOCX to DOTX", font=("Helvetica", 16, "bold"))  # Updated label
    ...
    def perform_conversion(self):
        for selected_file in self.selected_files:
            try:
                ...
                dotx_output_name = f"{docx_name}.dotx"

                # Check if the output file already exists
                output_file = os.path.join(self.output_directory, dotx_output_name)
                if os.path.exists(output_file):
                    # If the output file exists, generate a new filename with an incremental number
                    count = 1
                    while True:
                        new_name = f"{docx_name} ({count}).dotx"
                        new_path = os.path.join(self.output_directory, new_name)
                        if not os.path.exists(new_path):
                            dotx_output_name = new_name
                            break
                        count += 1

                # Convert DOCX to DOTX
                docx = Document(selected_file)
                docx.save(os.path.join(self.output_directory, dotx_output_name))

                # Display success message
                success_message = f"Conversion successful: {docx_basename} converted to {dotx_output_name}."
...
if __name__ == "__main__":
    root = tk.Tk()
    app = DOCXtoDOTXConverterUI(root)
    root.mainloop()
