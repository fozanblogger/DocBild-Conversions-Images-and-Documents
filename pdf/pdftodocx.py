...
class PDFtoDOCXConverterUI:
    def __init__(self, master):
        self.master = master
        self.master.title("PDF to DOCX")  # Updated title
        ...
        # Create widgets
        self.heading_label = tk.Label(self.master, text="PDF to DOCX", font=("Helvetica", 16, "bold"))  # Updated label
    ...
    def convert_files(self):
        for selected_file in self.selected_files:
            try:
                ...
                docx_output_name = f"{pdf_name}.docx"

                # Check if the output file already exists
                output_file = os.path.join(self.output_directory, docx_output_name)
                if os.path.exists(output_file):
                    # If the output file exists, generate a new filename with an incremental number
                    count = 1
                    while True:
                        new_name = f"{pdf_name} ({count}).docx"
                        new_path = os.path.join(self.output_directory, new_name)
                        if not os.path.exists(new_path):
                            docx_output_name = new_name
                            break
                        count += 1
                ...
                doc.save(os.path.join(self.output_directory, docx_output_name))

                # Display success message
                success_message = f"Conversion successful: {pdf_basename} converted to {docx_output_name}."
